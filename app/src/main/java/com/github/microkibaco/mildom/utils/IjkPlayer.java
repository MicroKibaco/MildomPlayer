...
/**
 * @author 杨正友(小木箱)于 2020/8/7 22 18 创建
 *
 * 邮箱: yangzy1@akulaku.com
 *
 * 电话: 18390833563
 *
 * 功能描述:
...
public class IjkPlayer extends BaseInternalPlayer {
    ...
    /**
     * ijkplayer的配置项设置 ijkMediaPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "mediacodec",
     * 1);
     *
    ...
    private IMediaPlayer.OnErrorListener mErrorListener =
            new IMediaPlayer.OnErrorListener() {
                private IMediaPlayer mMp;
                private int mFramework_err;
                private int mImpl_err;

                @Override
                public boolean onError(IMediaPlayer mp, int frameworkErr, int implErr) {
                    mMp = mp;
                    mFramework_err = frameworkErr;
                    mImpl_err = implErr;
                    PLog.d(TAG, "Error: " + frameworkErr + "," + implErr);
                    updateStatus(STATE_ERROR);
                    mTargetState = STATE_ERROR;
